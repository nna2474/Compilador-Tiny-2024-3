...
public class Generador {
	...
	private static void generarOperacion(NodoBase nodo){
		...
		switch(n.getOperacion()){
			...
			case	entre:	UtGen.emitirRO("DIV", UtGen.AC, UtGen.AC1, UtGen.AC, "op: /");
							break;
			case	mod:	UtGen.emitirRO("DIV", UtGen.AC2, UtGen.AC1, UtGen.AC, "op: mod divir (a/b)");
							UtGen.emitirRO("MUL", UtGen.AC2, UtGen.AC, UtGen.AC2, "multiplicar (a/b)*b");
							UtGen.emitirRO("SUB", UtGen.AC, UtGen.AC1, UtGen.AC2, "restar a-(a/b)*b");
							break;
			...
			case	diferente:
							...
							break;
			case	and:
							UtGen.emitirRM("JEQ", UtGen.AC, 3, UtGen.PC, "voy tres instrucciones mas alla if false (AC==0)");
							UtGen.emitirRM("JEQ", UtGen.AC1, 2, UtGen.PC, "voy dos instrucciones mas alla if false (AC==0)");
							UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "caso de verdadero (AC=1)");
							UtGen.emitirRM("LDA", UtGen.PC, 1, UtGen.PC, "Salto incodicional a direccion: PC+1 (es verdadero evito colocarlo falso)");
							UtGen.emitirRM("LDC", UtGen.AC, 0, UtGen.AC, "caso de falso (AC=0)");
							break;
			case	or:
							UtGen.emitirRM("JEQ", UtGen.AC, 2, UtGen.PC, "voy dos instrucciones mas alla if false (AC==0)");
							UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "caso de verdadero (AC=1)");
							UtGen.emitirRM("LDA", UtGen.PC, 4, UtGen.PC, "Salto incodicional a direccion: PC+4 (es verdadero evito colocarlo false)");
							UtGen.emitirRM("JEQ", UtGen.AC1, 2, UtGen.PC, "voy dos instrucciones mas alla if false (AC==0)");
							UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "caso de verdadero (AC=1)");
							UtGen.emitirRM("LDA", UtGen.PC, 1, UtGen.PC, "Salto incodicional a direccion: PC+1 (es verdadero evito colocarlo false)");
							UtGen.emitirRM("LDC", UtGen.AC, 0, UtGen.AC, "caso de falso (AC=0)");
							break;
			case	not:
							UtGen.emitirRM("JEQ", UtGen.AC, 2, UtGen.PC, "voy dos instrucciones mas alla if false (AC==0)");
							UtGen.emitirRM("LDC", UtGen.AC, 0, UtGen.AC, "si es verdadero (AC=0)");
							UtGen.emitirRM("LDA", UtGen.PC, 1, UtGen.PC, "Salto incodicional a direccion: PC+1 (es verdadero evito colocarlo false)");
							UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "si es falso (AC=1)");
							break;
			default:
	...
	private static void generarFor(NodoBase nodo){
		/*
		NodoRepeat n2 = (NodoRepeat)nodo;
		int localidadSaltoInicio1;
		if(UtGen.debug)	UtGen.emitirComentario("-> for");
		localidadSaltoInicio1 = UtGen.emitirSalto(0);
		UtGen.emitirComentario("for: inicio de la condición");
		/* Genero el cuerpo del repeat */
		/*generar(n2.getCuerpo());
		/* Genero el codigo de la prueba del repeat */
		/*generar(n2.getPrueba());
		UtGen.emitirRM_Abs("JEQ", UtGen.AC, localidadSaltoInicio1, "repeat: jmp hacia el inicio del cuerpo");
		if(UtGen.debug)	UtGen.emitirComentario("<- repeat");
		*/
